...
#add header
with open(userpath+'/filter1.csv', 'r') as incsv, open(userpath+'/combined_file.csv', 'w') as outcsv:
...
#sort by sscount?
flistsort = pd.read_csv(userpath+'/filter1.csv', sep=',', usecols=[0,1,2,3,4])
    
#new file with only sequences of probes for calculating free energies using oligoscreen
flistsort2 = pd.read_csv(userpath+'/filter1.csv', usecols=[3])
flistsort2.to_csv(userpath+'/Sorted_list.csv', index=False)

with open(userpath+"/Sorted_list.csv",'r') as f, open(userpath+"/updated_test.lis",'w') as f1:
...
with open(userpath+'/mb_picks.csv') as ff:
    ...
    for item in fseq:
        ...
        if stem1 in seq_slc or stem2 in seq_slc or stem3 in seq_slc:
            stem = 'GCACG'
